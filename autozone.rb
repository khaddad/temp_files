class NECB2011
  ...
  def necb_autozone_and_autosystem(model: nil, runner: nil, use_ideal_air_loads: false, system_fuel_defaults:)
    ...
    if use_ideal_air_loads == true
      ...
      chw_loop_defined = false
      system_zone_array.each_with_index do |zones, system_index|
        # skip if no thermal zones for this system.
        next if zones.empty?
        if(!chw_loop_defined)
          if(system_index == 2 || system_index == 5 || system_index == 6)
            # Chilled Water Plant
            chw_loop = OpenStudio::Model::PlantLoop.new(model)
            chiller1, chiller2 = BTAP::Resources::HVAC::HVACTemplates::NECB2011.setup_chw_loop_with_components(model, chw_loop, system_fuel_defaults['chiller_type'])
            # Condenser System
            cw_loop = OpenStudio::Model::PlantLoop.new(model)
            ctower = BTAP::Resources::HVAC::HVACTemplates::NECB2011.setup_cw_loop_with_components(model, cw_loop, chiller1, chiller2)
            chw_loop_defined = true
          end
        end
        case system_index
        when 0, nil
        ...
        when 1
#          add_necb2011_system(model,
#                             zones,
#                            "Makeup-Air Unit",
#                             system_fuel_defaults['boiler_fueltype'],
#                            true,
#                             "CV",
#                           "MZ",
#                            system_fuel_defaults['mau_heating_coil_type'],
#                            "DX",
#                             "PTAC",
#                            "Electric",
                            "DX",
                            system_fuel_defaults['baseboard_type'],
                            hw_loop,
                            chw_loop)
        ...
        when 2
          add_sys2_FPFC_sys5_TPFC(model, zones, system_fuel_defaults['boiler_fueltype'], system_fuel_defaults['chiller_type'], 'FPFC', system_fuel_defaults['mau_cooling_type'], hw_loop, chw_loop)
        when 3
          puts "test111",system_fuel_defaults['mau_heating_coil_type']
          #add_necb2011_system(model,
          #                    zones,
          #                   "NECB System 3 PSZ",
          #                  system_fuel_defaults['boiler_fueltype'],
        ...
        when 5
          add_sys2_FPFC_sys5_TPFC(model, zones, system_fuel_defaults['boiler_fueltype'], system_fuel_defaults['chiller_type'], 'TPFC', system_fuel_defaults['mau_cooling_type'], hw_loop, chw_loop, chw_loop)
        when 6
          add_sys6_multi_zone_built_up_system_with_baseboard_heating(model, zones, system_fuel_defaults['boiler_fueltype'], system_fuel_defaults['heating_coil_type_sys6'], system_fuel_defaults['baseboard_type'], system_fuel_defaults['chiller_type'], system_fuel_defaults['fan_type'], hw_loop, chw_loop)
        when 7
