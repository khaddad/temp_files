...
begin
  ...
  def add_necb2011_system(model, zones, sys_name, boiler_fuel_type, mau, air_sys_flow, air_sys_zoning, air_sys_heating_coil_type,
                          air_sys_cooling_coil_type, zone_sys_type, zone_htg_coil_type, zone_clg_coil_type, zone_baseboard_type,
                          hw_loop, chw_loop)

    # zones: Thermal zones array
    # sys_name: part of full system name
    # boiler_fuel_type: "none"; "NaturalGas"; "Electricity"; "PropaneGas"; "FuelOil#1"; "FuelOil#2"; "Coal"; "Diesel"; "Gasoline"; "OtherFuel1"
    # mau: true (100% outdoor air); false (outdoor air < 100%)
    # air_sys_flow: "none"; "CV";
    # air_sys_zoning: "none"; "MZ"
    # air_sys_cooling_coil_type: "none"; "DX"
    # air_sys_heating_coil_type: "none"; "Hot Water"; 'Electric'
    # zone_sys_type: "none"; "PTAC"
    # zone_htg_coil_type: "none"; "Electric"
    # zone_clg_coil_type: "none"; "DX"
    # zone_baseboard_heating_type: "none"; "Hot Water"; "Electric"
    # hw_loop: Hot-water loop
    # chw_loop: Chilled-water loop

    # define always on and always off schedules
  ...
  def create_zone_sys(model, zone, hw_loop, chw_loop, zone_sys_type, zone_htg_coil_type, zone_clg_coil_type, zone_baseboard_type)

    # zone: Thermal zone
    # hw_loop: Hot-water loop
    # chw_loop: Chilled-water loop
    # zone_sys_type: "none"; "PTAC"
    # zone_htg_coil_type: "none"; "Electric"; "Hot Water"
    # zone_clg_coil_type: "none; "DX"; "Chilled Water""
    ...
    # Zone heating coil
    if(zone_htg_coil_type == "Electric")
      zone_htg_coil = OpenStudio::Model::CoilHeatingElectric.new(model, always_on)
    elsif(zone_htg_coil_type == "Hot Water")
      zone_htg_coil = OpenStudio::Model::CoilHeatingWater.new(model, always_on)
      hw_loop.addDemandBranchForComponent(zone_htg_coil)
    end
    ...
    # Zone system
    zone_sys = nil
    if(zone_sys_type == "PTAC")
      zone_htg_coil.setAvailabilitySchedule(always_off)
      zone_sys = OpenStudio::Model::ZoneHVACPackagedTerminalAirConditioner.new(model, always_on, zone_fan, zone_htg_coil, zone_clg_coil)
      zone_sys.setName("#{zone.name} PTAC")
    end
    if zone_sys then zone_sys.addToThermalZone(zone) end

    # Zone baseboards
    if(zone_baseboard_type == "Electric")
  ...
  def create_air_sys_multi_zone(model, zones, sys_name, hw_loop, chw_loop, air_sys_flow, mau, air_sys_htg_coil_type,
    ...
    # chw_loop: Chilled-water loop
    # air_sys_flow: "CV"
    # mau: true (100% oudoor air); false (outdoor air < 100%)
    # air_sys_htg_coil_type: "none"; "Hot Water'; "Electric"
    # air_sys_clg_coil_type: "none"; "DX"
    ...
    # set up air loop
    air_sys_loop = OpenStudio::Model::AirLoopHVAC.new(model)
    air_sys_loop.setName(sys_name)
    load_to_size_on = "Sensible"
    all_oa_clg = false
    all_oa_htg = false
    if mau
      load_to_size_on = "VentilationRequirement"
      all_oa_clg = true
      all_oa_htg = true
    end
    air_loop_sizing = air_sys_loop.sizingSystem
    air_loop_sizing.setTypeofLoadtoSizeOn(load_to_size_on)
    ...
    air_loop_sizing.setAllOutdoorAirinCooling(all_oa_clg)
    air_loop_sizing.setAllOutdoorAirinHeating(all_oa_htg)
    ...
    # supply Fan
    if(air_sys_flow == "CV")
      air_sys_fan = OpenStudio::Model::FanConstantVolume.new(model, always_on)
    end

    # heating coil
    air_sys_htg_coil = nil
    if(air_sys_htg_coil_type == "Electric")
    ...
    if mau
      ...
      sat_sch.setName("#{sys_name} Supply Air Temp")
      sat_sch.defaultDaySchedule.setName("#{sys_name} Supply Air Temp Default")
      ...
      setpoint_mgr.addToNode(air_sys_loop.supplyOutletNode)
    end
  ...
  def create_air_sys_single_zone(model, zone, sys_name, hw_loop, chw_loop, air_sys_flow, mau, air_sys_htg_coil_type,
                                 air_sys_clg_coil_type)
    ...
    # set up air loop
    air_sys_loop = OpenStudio::Model::AirLoopHVAC.new(model)
    air_sys_loop.setName("#{zone.name} #{sys_name}")
    load_to_size_on = "Sensible"
    all_oa_clg = false
    all_oa_htg = false
    if mau
    ...
    elsif(air_sys_htg_coil_type == "Gas")
      air_sys_htg_coil = OpenStudio::Model::CoilHeatingGas.new(model, always_on)
    elsif(air_sys_htg_coil_type == "DX")
      air_sys_htg_coil = OpenStudio::Model::CoilHeatingDXSingleSpeed.new(model)
      supplemental_htg_coil = OpenStudio::Model::CoilHeatingElectric.new(model, always_on)
      air_sys_htg_coil.setMinimumOutdoorDryBulbTemperatureforCompressorOperation(-10.0)
    end
    ...
    if mau
      ...
      setpoint_mgr_single_zone_reheat.addToNode(air_sys_loop.supplyOutletNode)
    end
  ...
  def add_sys2_FPFC_sys5_TPFC(model, zones, boiler_fueltype, chiller_type, fan_coil_type, mua_cooling_type, hw_loop, chw_loop)
...
#    ctower = BTAP::Resources::HVAC::HVACTemplates::NECB2011.setup_cw_loop_with_components(model, cw_loop, chiller1, chiller2)
  ...
  def add_sys6_multi_zone_built_up_system_with_baseboard_heating(model, zones, boiler_fueltype, heating_coil_type, baseboard_type, chiller_type, fan_type, hw_loop, chw_loop)
